...
class UserFavPlayersActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val otherPlayer: MutableSet<PlayerID> = getAllPlayer().minus(currentUser.favPlayer) as MutableSet<PlayerID>
...
fun addToFavPlayer(view: View, player: PlayerID) {
    ...
    Snackbar.make(view, "${getPlayerById(player)?.fullName} is added to Favorite", Snackbar.LENGTH_SHORT)
...
fun removeFromFavPlayer(view: View, player: PlayerID) {
    ...
    Snackbar.make(view, "${getPlayerById(player)?.fullName} is removed from Favorite", Snackbar.LENGTH_SHORT)
